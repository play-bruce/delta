...
function clearQueue(fn) {
    const findFn = arguments.length > 0;
    while (queue.length) {
        const node = queue.pop()[1]; // remove current node
        node.fn(node.emit);
        if (findFn && node.fn === fn) break;
    }
}

let deferActive = false;
function deferClearQueue() {
    if (!deferActive) {
        deferActive = true;
        setTimeout((
            () => clearQueue(), deferActive = false
        ), 1);
    }
}
                ...
                node.dependants.forEach(dependantNode => {
                    ...
                    while (i--) {
                        if (queue[i][1].order < dependantNode.order) {
                            queue.splice(i, 0, [node, dependantNode]);
                            break;
                        }
                    }
                    if (!queue.length) {
                        queue.push([node, dependantNode]);
                    }
                });
                clearQueue();
            },
...
let a;
let b;
const $a = node(emit => {emit(0), a = emit});
const $b = node(emit => {emit(0), b = emit});

// const $sum = $a(emit => emit($a() + $b()));
// $b($sum);
const $sum = node($a, $b, emit => emit($a() + $b()));
